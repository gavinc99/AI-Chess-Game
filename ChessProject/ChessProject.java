...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
  ...
  AIAgent agent;
  //static int option;
  static int n; //this is when I selct the option from GUI it is 0,1 or 2
  int moveCounter;
...
//lab assist queen addition
  ...
  private Boolean pieceMove(int x, int y) {
    ...
    /*
      ...
      its starting position in which  it can move either one or two squares or it has already moved and the it can only
    ...
    */
    ...
    private Stack getWhitePawnSquares(int x, int y, String piece){ //here I just the coordinates for X and also Y and then piece
      Stack moves = new Stack(); // creating moves stack because I need to store all of the moves in order to add the valid moves
      Square startingSquare = new Square(x, y, piece);
        ...
        //here we have the temporary x1,x2,y1 and y2
        int tmpx1 = x+1;
        int tmpx2 = x-1;
        int tmpy1 = y+1;
        int tmpy2 = y+2;

        //first listing all moves for the pawn, it will either move 1 or 2 squares down at the start and then left/right down if taking a piece
        //This means that we have 4 possibilites, in total which are listed below

        //Down by 2 I put it first since it will most likely be the first option as only on the first move pawn can move up 2
        Square tmpSquare3 = new Square(x, tmpy2, piece);
        //Simply goes down one
        Square tmpSquare = new Square(x, tmpy1, piece);
        // Down one and also one to the Right so this is only possible if the pawn is attacking
        Square tmpSquare1 = new Square(tmpx1, tmpy1, piece);
        //Down and also Left this also can only happen if the pawn is attacking
        Square tmpSquare2 = new Square(tmpx2, tmpy1, piece);


        //Here if the pawn is in its start postion
        if (y == 1) {
          //so Square 1 here
              validM = new Move(startingSquare, tmpSquare3); //declaring the starting square here with the temporary temp square 3
              if (!piecePresent(((tmpSquare3.getXC() * 75) + 20), //if there is no piece in the squre
              (((tmpSquare3.getYC() * 75) + 20))) && !piecePresent(((tmpSquare.getXC() * 75) + 20), //this code is based of looking at pre-existing code for the KING
              (((tmpSquare.getYC() * 75) + 20)))) {
                  moves.push(validM); //so here pushing new validm into the moves which I declared above, NOTE I am pushing the validm into my stack created on line 169
              }

              else {
                  if (!(tmpx1 > 7)) { //this is similar as above but here is the check if the x is NOT bigger than 7. if it was bigger then it would be off the board
                      if (piecePresent(((tmpSquare1.getXC() * 75) + 20), //checking if there is a piece present down and also right so tmp square 1 is to check if we can move down and to the right which we can only do if we are attacking
                      (((tmpSquare1.getYC() * 75) + 20)))) {
                          if (checkWhiteOponent(((tmpSquare1.getXC() * 75) + 20), //goes to checkWhiteOponent tmp1 again to see if there is a black piece to attack
                          (((tmpSquare1.getYC() * 75) + 20)))) {
                              moves.push(validM); //and then pushes into stack
                          }
                      }
                  }

                  if (!(tmpx2 < 0)) { //same as above just in this case if its NOT less than 0
                      if (piecePresent(((tmpSquare2.getXC() * 75) + 20),
                      (((tmpSquare2.getYC() * 75) + 20)))) { //this tmp square is for down and left
                          if (checkWhiteOponent(((tmpSquare2.getXC() * 75) + 20), (((tmpSquare2.getYC() * 75) + 20)))) {
                              moves.push(validM); //again just pushes to stack
                          }
                      }
                  }
              }
          } // this bracket closes if (y==1)

//this is very similar overall to what I have above just iterating thru differnt possibilities

//in this case tmpy1 is just Y+1 as I made the temporary positions above
          if (!(tmpy1 > 7)) { //checking if my temp is not bigger than 7 if it is bigger than it is off the board
              validM = new Move(startingSquare, tmpSquare);
              if (!piecePresent(((tmpSquare.getXC() * 75) + 20),
              (((tmpSquare.getYC() * 75) + 20)))) {
                  moves.push(validM);
              }

              if (!(tmpx1 > 7)) {
                  validM2 = new Move(startingSquare, tmpSquare1); //here it checks to be sure its a black piece to be attacked
                  if (piecePresent(((tmpSquare1.getXC() * 75) + 20), // check if piece is present
                  (((tmpSquare1.getYC() * 75) + 20)))) {
                      if (checkWhiteOponent(((tmpSquare1.getXC() * 75) + 20), (((tmpSquare1.getYC() * 75) + 20)))) { // making sure I am attacking enemy piece which has to be black
                          moves.push(validM2); // again simply into stack
                      }
                  }
              }


              //again checking all the different possibilites
              if (!(tmpx2 < 0)) {
                  validM3 = new Move(startingSquare, tmpSquare2); //defining a new move
                  if (piecePresent(((tmpSquare2.getXC() * 75) + 20), //again checking if the piece is present
                  (((tmpSquare2.getYC() * 75) + 20)))) {
                      if (checkWhiteOponent(((tmpSquare2.getXC() * 75) + 20), (((tmpSquare2.getYC() * 75) + 20)))) { //making sure I will atack an enemy so BLACK piece
                          moves.push(validM3);
                      }
                  }
              }
          }
          return moves; // so here we are returning all move possibilities which I stored in moves and in the temp variables
      }
    ...
    */
    private Stack getKingSquares(int x, int y, String piece){ //in this case the valid moves are called in as moves which are stored in a stack
      Square startingSquare = new Square(x, y, piece);
    ...
    */
      if(!((tmpx1 > 7))){ //functions like this validate the move
        /*
          ...
          in the grid.
        */
        System.out.println("This print out test is for get king");
        ...
        if(checkSurroundingSquares(tmp)){
          ...
          if(!piecePresent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){
            ...
            if(checkWhiteOponent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){ //here a checkwhite opponent only looks for black pieces as they are enemies
              moves.push(validM);
            }
          }
        }
    ...
    */
    private Stack getKnightMoves(int x, int y, String piece){
      ...
      for(int i=0;i < 8;i++){
        ...
        else if(piecePresent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){ //checks if piece is present
          if(piece.contains("White")){
            if(checkWhiteOponent(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20))){//checks if the opponent is present
    ...
    */
      ...
      //this is made to find all of the black pieces and is based on the prexisting find whitepieces above
      private Stack findBlackPieces() {
        ...
        for (int i = 0; i < 600; i += 75) { //here as our board is 600 with each square being 75 we go up using i++ all the way too 600
            for (int j = 0; j < 600; j += 75) {
                y = i / 75; //here again it is in 75's as each square is 75
                x = j / 75;
                Component tmp = chessBoard.findComponentAt(j, i);
                if (tmp instanceof JLabel) {
                    chessPiece = (JLabel) tmp;
    ...
    private void makeAIMove(){
      ...
      System.out.println ("Start if the method make AIMove");
      /*
      ...
      */
      ...
      layeredPane.repaint();
      Stack white = findWhitePieces(); //this is to find all of the white pieces Stack is a last in first out structure
      Stack black = findBlackPieces(); //this simply finds all black pieces
      Stack completeMoves = new Stack();
      Move tmp;
      while(!white.empty()){
        ...
        */
        if(tmpString.contains("Knight")){
          tmpMoves = getKnightMoves(s.getXC(), s.getYC(), s.getName()); //tmp move is also a stack as we are all of the possible moves in a stack.
        }
    ...
    */
    ...
    System.exit(0);
      ...
      */
      ...
      //if  (n == 0) {
        System.out.println ("The option was selected put anyrthign");
        ...
        while(!completeMoves.empty()){
         ...
         // NOTE : intially I was trying to implement the choice using the switch statement which did WORK but when I tried to add the Greedy best I could NOT GET IT TO WORK so I switched to using if statements below

         /*switch (n) {
                   case 0  :  agent.randomMove(testing);
                   case 1  :  agent.nextBestMove(testing, black);
                   //case 1 "nextBest" -> selectedMove = agent.nextBestMove(testing);
                   //case 2 "twoLevelsDeep" -> selectedMove = agent.twoLevelsDeep(testing);
         ...
         if(n==0){
           ...
           selectMove = agent.nextBestMove(testing);
         }
         else{
           selectMove = agent.twoLevelsDeep(testing);
         }
         */
  ...
  /*
    This method checks if a piece is a black piece (it is an opponent)
  */
  ...
  private String getPieceName(int x, int y) {
	...
	*/
    ...
    //if(pieceName.equals("BlackQueen")){
      //validMove = true;        //queen can move wherever she wants to go now
    //}



//Error in code below made the white queen only do bishop movement but not rook moves
    ...
    if(pieceName.contains("Bishop")){
      ...
      if(((landingX < 0) || (landingX > 7)) || ((landingY < 0)||(landingY > 7))){
        ...
        if(Math.abs(startX-landingX)==Math.abs(startY-landingY)){
          if((startX-landingX <0)&&(startY-landingY < 0)){
            System.out.println ("Moving right and down");
            for(int i=0; i < distance; i++){
              if(piecePresent((initialX+(i*75)), (initialY+(i*75)))){
                inTheWay = true;
              }
            ...
            System.out.println ("Moving right and up");
            for(int i=0; i < distance; i++){
              if(piecePresent((initialX+(i*75)), (initialY-(i*75)))){
                inTheWay = true;
              }
            ...
            System.out.println ("Moving left and up");
            for(int i=0; i < distance;i++){
              if(piecePresent((initialX-(i*75)),(initialY-(i*75)))){
                inTheWay= true;
              }
            ...
            System.out.println ("Moving left and down");
            for(int i=0; i < distance;i++){
              if(piecePresent((initialX-(i*75)),(initialY+(i*75)))){
                inTheWay= true;
              }
      ...
      System.out.println ("Current bishop move is"+ validMove);
    }
    ...
    /*
          ...
          //validMove = true; Black pawn can also move wherever black pawn wants to go
      if (startY==6){ // THe pawn is making the first move here


/*
if the pawn is making its first movement
the pawn can move either by one square or by two squares... This is in the Y direction if we are moving up the board, and also there is no movement in the x direction
*/
    /*  if(((yMovement==1)||(yMovement == 2))&&(startY > landingY)&&(xMovement==0)){
          if (yMovement == 2){
            if((!piecePresent(e.getX(), e.getY()))&&(!piecePresent(e.getX(), (e.getY()+75)))){
            validMove = true; //here we need to put code to turn white black pawn into a queen...
            }
            if(!validMove){
        			int location=0;
        			if(startY ==0){
            ...
            else{                                                                   // here is start of the code  from white pawn
        			if(success){
        		...
        		}                                                                          //here is the end of the code from the white pawn
          }
          else{
            if(!piecePresent(e.getX(),e.getY())){
              validMove = true;
      ...
      else{ // Here the pawn is making all moves after the first move...
  ...
  // Issue lies here ---- This if statement should be inside if piece name is black pawn condition.
  // Since it was outside before, this was executing for white pawn move also and was adding a new pawn image in the screen all the time
  // Now I moved the whole if block inside the black pawn move condition and it worked
  ...
  if(progression){
    ...
    if(c instanceof JLabel){
      ...
      makeAIMove();
      System.out.println("THis test checks the makeai move");
    }
    }
	...
	*/
    public static void main(String[] args) {
        ...
        Object[] options = {"Random Move","Greedy best ","Two steps ahead"};
        n = JOptionPane.showOptionDialog(frame,"Choose your AI opponent","Introduction to AI Continuous Assessment", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE,null,options,options[2]);
        frame.makeAIMove();
